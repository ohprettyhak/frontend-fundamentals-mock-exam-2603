...
describe('useAvailableRooms', () => {
  test('isFilterComplete가 false이면 빈 배열을 반환한다', () => {
    const { availableRooms } = useAvailableRooms(rooms, [], baseFilters, false);
    expect(availableRooms).toEqual([]);
  });

  test('수용 인원 미달 회의실을 제외한다', () => {
    const filters = { ...baseFilters, attendees: 15 };
    const { availableRooms } = useAvailableRooms(rooms, [], filters, true);
  ...
  test('시간이 겹치는 예약이 있으면 해당 회의실을 제외한다', () => {
    ...
    const { availableRooms } = useAvailableRooms(rooms, reservations, baseFilters, true);
  ...
  test('층수 오름차순, 같은 층이면 이름순으로 정렬한다', () => {
    const { availableRooms } = useAvailableRooms(rooms, [], baseFilters, true);
